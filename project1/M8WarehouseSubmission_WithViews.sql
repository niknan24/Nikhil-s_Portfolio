...
CREATE OR REPLACE TABLE Dim_Store(
    ...
    ,DimLocationID_Store INTEGER CONSTRAINT FK_DimLocationIDStore FOREIGN KEY REFERENCES Dim_Location (DimLocationID)
...
INSERT INTO Dim_Store
(DimStoreID, DimLocationID_Store, SourceStoreID, StoreNumber, StoreManager)
    SELECT StoreID, DimLocationID, StoreID, StoreNumber, StoreManager

    FROM STAGE_STORE 
    INNER JOIN Dim_Location L ON STAGE_STORE.PostalCode = L.PostalCode;
...
CREATE OR REPLACE TABLE Dim_Reseller(
    ...
    ,DimLocationID_Reseller INTEGER CONSTRAINT FK_DimLocationIDReseller FOREIGN KEY REFERENCES Dim_Location (DimLocationID) 
...
INSERT INTO Dim_Reseller
(DimResellerID, DimLocationID_Reseller, ResellerID, ResellerName, ContactName, PhoneNumber, Email)
    SELECT ResellerID, DimLocationID, ResellerID, ResellerName, Contact, PhoneNumber, EmailAddress

    FROM STAGE_RESELLER 
    INNER JOIN Dim_Location L ON STAGE_RESELLER.PostalCode = L.PostalCode;

---Create Dim_Customer
CREATE OR REPLACE TABLE Dim_Customer(
    DimCustomerID VARCHAR(255) CONSTRAINT PK_DimCustomerID PRIMARY KEY NOT NULL --Surrogate Key
    ,DimLocationID_Customer INTEGER CONSTRAINT FK_DimLocationIDCustomer FOREIGN KEY REFERENCES Dim_Location (DimLocationID) 
...
(    DimCustomerID 
    ...
    ,CustomerLastName 
    ,CustomerGender)
    SELECT CustomerID, DimLocationID, CustomerID, CONCAT(FirstName, ' ', LastName), FirstName, LastName, Gender
    
    FROM STAGE_CUSTOMER
    INNER JOIN Dim_Location L ON STAGE_CUSTOMER.PostalCode = L.PostalCode;
...
INSERT INTO Fact_SRCSalesTarget
 ...
 ,SalesTargetAmount
    ...
    SELECT
        ...
        ,NVL(DimResellerID,'Unknown') AS DimResellerID
        ,NVL(DimChannelID, -1) AS DimChannelID
        ,CAST(CASE WHEN LEFT(DATE_PKEY, 4)=2014 THEN 2014 ELSE 2013 END as int) AS DimTargetTypeDateID
        ,TargetSalesAmount
    FROM STAGE_TARGET_CHANNELRESELLERSTORE AS CRS
        ...
        LEFT OUTER JOIN Dim_Reseller R ON SH.ResellerID = R.DimResellerID
        LEFT OUTER JOIN Dim_Date D ON CRS.Year = D.Date;
...
(
  ...
  ,ProductTargetSalesQuantity 
    ...
    SELECT
    NVL(DimProductID, -1) as Dim_ProductID
    ,CAST(CASE WHEN LEFT(DATE_PKEY, 4)=2014 THEN 2014 ELSE 2013 END as int) AS DimTargetDateID
    ,SalesQuantityTarget
    FROM Dim_Product AS P
    INNER JOIN STAGE_TARGET_PRODUCT TP ON P.DimProductID = TP.ProductID
    LEFT OUTER JOIN Dim_Date D ON TP.Year = D.Date;
...
(
  ...
  ,SaleTotalProfit
    ...
    SELECT
        ...
        ,NVL(DimStoreID, -1) AS DimStoreID
        ,NVL(DimResellerID, 'Unknown') AS DimResellerID
        ,NVL(DimCustomerID, 'Unknown') AS DimCustomerID
        ,NVL(ChannelID, -1) AS DimChannelID
        ,NVL(DATE_PKEY, -1) AS DimSaleDateID
        ,NVL(DimLocationID,-1) AS DimLocationID
        ,NVL(SalesDetailID,-1) AS SalesHeaderID
        ,NVL(SalesDetailID, -1) AS SalesDetailID
        ,SalesAmount
        ,SalesQuantity
        ,ProductRetailPrice --Unit Price 
        ,ProductCost --Extended Cost
        ,((ProductRetailPrice - ProductCost) * SalesQuantity) --Total profit formula 
     ...
     INNER JOIN Dim_Location L ON S.DimLocationID_Store = L.DimLocationID
       LEFT JOIN Dim_Reseller R ON L.DimLocationID = R.DimLocationID_Reseller
     INNER JOIN Dim_Date D ON dateadd(year, 2000, SH.DATE) = D.DATE
        OR SH.DATE = D.DATE
     INNER JOIN STAGE_SALESDETAIL SD ON SH.SalesHeaderID = SD.SalesHeaderID
       INNER JOIN Dim_Product P ON SD.ProductID = P.DimProductID;

--Create pass-through view for Dim_Product

CREATE OR REPLACE SECURE VIEW Dim_Product_View
    AS
    SELECT 
    DimProductID 
	,ProductID 
...
---Create pass-through view for Dim_Location

CREATE OR REPLACE SECURE VIEW Dim_Location_View
    AS
    SELECT
    DimLocationID
    ,Address
...
---Create pass-through view for Dim_Store

CREATE OR REPLACE SECURE VIEW Dim_Store_View
    AS
    SELECT
    DimStoreID 
    ,DimLocationID_Store
...
---Create pass-through view for Dim_Reseller

CREATE OR REPLACE SECURE VIEW Dim_Reseller_View
    AS 
    SELECT
    DimResellerID 
    ,DimLocationID_Reseller 
...
---Create pass-through view for Dim_Customer

CREATE OR REPLACE SECURE VIEW Dim_Customer_View
    AS
    SELECT
    DimCustomerID 
    ,DimLocationID_Customer 
...
---Create pass-through view for Dim_Channel
CREATE OR REPLACE SECURE VIEW Dim_Channel_View
    AS
    SELECT
    DimChannelID 
    ,ChannelID 
...
---Create pass-through view for Dim_Date 

CREATE OR REPLACE SECURE VIEW Dim_Date_View
    AS
    SELECT
    DATE_PKEY,
    FULL_DATE_DESC,
...
---Create pass-through view for Fact_SRCSalesTarget

CREATE OR REPLACE SECURE VIEW Fact_SRCSalesTarget_View
    AS
    SELECT
    DimStoreID 
 ,DimResellerID 
...
---Create pass-through view for Fact_ProductSalesTarget

CREATE OR REPLACE SECURE VIEW Fact_ProductSalesTarget_View
    AS
    SELECT
    DimProductID
  ,DimTargetDateID
...
---Create pass-through view for Fact_SalesActual

CREATE OR REPLACE SECURE VIEW Fact_SalesActual_View
    AS
    SELECT
    DimProductID
  ,DimStoreID
...
--- Question 1: Custom view for sales and profit values comparison for Stores 5 and 8
CREATE OR REPLACE SECURE VIEW OverallSalesProfitComparison_View
AS
SELECT 
    StoreNumber,
    SUM(SaleTotalProfit) AS TotalProfit,
    SUM(SaleQuantity) AS TotalItemsSold
    FROM Fact_SalesActual AS FSA
    INNER JOIN Dim_Store S ON FSA.DimStoreID = S.DimStoreID
    WHERE StoreNumber = '8' OR StoreNumber = '5'
    GROUP BY StoreNumber
    ORDER BY TotalProfit DESC;
  
---Question 1: Total Sales and Average Profit Margins by various product categories for Store 5 and Store 8

CREATE OR REPLACE SECURE VIEW SalesAVGProfitMarginsComparison_View
AS
SELECT
    StoreNumber,
    ProductCategory,
    SUM(SaleAmount) AS TotalSalesAmount,
    AVG(ProductProfitMarginUnitPercent) AS AverageProductCategoryProfitMargin
FROM Fact_SalesActual AS FSA
    INNER JOIN Dim_Store S ON FSA.DimStoreID = S.DimStoreID
    INNER JOIN Dim_Product P ON FSA.DimProductID = P.DimProductID
    WHERE StoreNumber = '5' OR StoreNumber = '8'
    GROUP BY StoreNumber, ProductCategory
    ORDER BY AverageProductCategoryProfitMargin DESC;

---Question 3: Sales/cost/profit by day of week---

CREATE OR REPLACE SECURE VIEW ProfitByDayWeek AS
SELECT
    StoreNumber
    ,DAY_NAME
    ,AVG(SaleAmount) AS AvgSaleAmount
    ,AVG(ProductCost * SaleQuantity) AS ExtendedCost
    ,AVG(SaleTotalProfit) AS AvgTotalProfit
...
JOIN Dim_Store S ON S.DimStoreID = FSA.DimStoreID
WHERE StoreNumber = '8' OR StoreNumber = '5'
GROUP BY StoreNumber, DAY_NAME; 
